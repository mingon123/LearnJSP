	...
	$(function(){
		...
		$('#id_check').click(function(){
			if(!/^[A-Za-z0-9]{4,12}$/.test($('#id').val())){
				...
				$('#id').val('').focus();
				return; // submit아니기 때문에 return하면 됨
			}
			// 서버와 통신
			$.ajax({
		...
		$('#register_form #id').keydown(function(){ // #register_form 생략 가능
			idChecked = 0;
			$('#message_id').text('');
		});
		...
		$('#email_check').click(function(){
			if(!/^\w+@\w+\.\w+$/.test($('#email').val())){ // 1개이상의 문자+@+1개 이상의 문자+.+1개 이상의 문자
		...
		<form id="register_form" action="registerUser.do" method="post">
			<ul>
				...
				<li>
					...
					<input type="email" name="email" id="email" maxlength="50" class="input-check" autocomplete="off"><!-- 크롬은 적용되지만 엣지는 적용안됨 -->
