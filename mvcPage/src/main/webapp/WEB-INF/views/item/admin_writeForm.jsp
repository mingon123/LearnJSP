...
<script type="text/javascript"> // jquery를 사용하지 않고 순수 js로 등록
	window.onload=function(){
		const myForm = document.getElementById('write_form');
		// 이벤트 연결
		myForm.onsubmit=function(){
			const radio = document.querySelectorAll('input[type=radio]:checked'); // 체크된 값 읽어옴
			if(radio.length < 1){ // 선택하지 않음
				alert('상품표시여부를 지정하세요');
				return false;
			}
...
<div class="page-main">
	...
	<div class="content-main">
		<h2>상품등록</h2>
		<form action="adminWrite.do" method="post" id="write_form" enctype="multipart/form-data">
			<ul>
				<li>
					<label>상품표시여부</label>
