$(function(){
		...
		$.ajax({
			...
			success:function(param){
				...
				$(param.list).each(function(index,item){
					...
					output += '<div class="sub-item">'; // 전체 item은 보여지면서 sub_item(내용)은 보여지지 않게 하기 위해
					output += '<p>' + item.re_content + '</p>';
					
					if(item.re_modifydate){
						output += '<span class="modify-date">최근 수정일 : ' + item.re_modifydate + '</span>';
					}else{
						output += '<span class="modify-date">수정일 : ' + item.re_date + '</span>';
					}
					// 로그인한 회원번호와 작성자의 회원번호 일치 여부
					if(param.user_num == item.mem_num){
						// 로그인한 회원번호와 작성자 회원번호 일치
						// 앞 문자열 , 뒤 문자열 사이에 re_num ''가 문자열
		...
		$.ajax({
			...
			success:function(param){
				if(param.result == 'logout'){
					...
					// 폼 초기화
					initForm();
					// 댓글 작성이 성공하면 새로 입력한 글을 포함해서 첫 번째 페이지의 게시글 목록을 다시 호출함
					selectList(1);
				}else{
					alert('댓글 등록 오류 발생');
				}			
	...
	===========================*/
	// 댓글 수정버튼 클릭시 수정폼 노출
	$(document).on('click','.modify-btn',function(){ // 수정버튼처럼 동적으로 만들어진 미래의 태그이므로 document.on으로 연결해야 함
		// 댓글 번호
