...
public class OrderDAO {
	...
	public void insertOrder(OrderVO order, List<OrderDetailVO> orderDetailList) throws Exception{
		...
		try {
			...
			// 주문정보 처리 (order 테이블)
			sql = "INSERT INTO zorder (order_num,order_total,payment,receive_name,receive_post,receive_address1,receive_address2,receive_phone,notice,mem_num) "
				+ "VALUES (?,?,?,?,?,?,?,?,?,?)";
	...
	public int getOrderCountByMem_num(String keyfield,String keyword,long mem_num)throws Exception{
		...
		try {
			...
			if(keyword!=null && !"".equals(keyword)) {
				...
				if(keyfield.equals("2")) sub_sql += "AND order_num = ?";
			}
			// SQL문 작성
	...
	public List<OrderVO> getListOrderByMem_num(int start, int end, String keyfield, String keyword, long mem_num)throws Exception{
		...
		try {
			...
			sql = "SELECT * FROM (SELECT a.*,rownum rnum FROM ("
				+ "SELECT * FROM zorder JOIN (SELECT order_num, "
				+ "LISTAGG(item_name,',') WITHIN GROUP (ORDER BY item_name) item_name "
				+ "FROM zorder_detail GROUP BY order_num) USING(order_num) "
				+ "WHERE mem_num=? "+sub_sql+ "ORDER BY order_num DESC)a) "
				+ "WHERE rnum >= ? AND rnum <= ?";
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터 바인딩
		...
		try {
			...
			if(order.getStatus() == 5) {
				...
				for (int i=0;i<detailList.size();i++) {
					...
					if(i % 1000 == 0) { // 1000건마다 DB로 보냄
						pstmt2.executeBatch();
					} // end of if
				} // end of for
				pstmt2.executeBatch(); // 메모리에 남은 나머지 데이터도 DB에 보냄
			} // end of if
			
			// 모든 SQL문이 정상적으로 실행되었을 경우
			conn.commit();
		} catch (Exception e) {
			// 하나라도 SQL문에 오류가 있는 경우
			conn.rollback();
			throw new Exception(e);
		} finally {
			DBUtil.executeClose(null, pstmt2, null);
