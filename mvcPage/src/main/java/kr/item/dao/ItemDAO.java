...
public class ItemDAO {
	...
	public void insertItem(ItemVO item)throws Exception{
		...
		try {
			...
			pstmt.setInt(++cnt, item.getStatus()); // 원래는 옵션이나 부가적인 정보들을 더 많이 넣어야 함
			// SQL문 실행
			pstmt.executeUpdate();			
		}catch (Exception e) {
	...
	public List<ItemVO> getListItem(int start, int end, String keyfield, String keyword, int status) throws Exception{
		...
		try {
			...
			sql = "SELECT * FROM (SELECT a.*, rownum rnum FROM (SELECT * FROM zitem WHERE status > ? " 
				+ sub_sql + " ORDER BY item_num DESC)a) WHERE rnum >= ? AND rnum <= ?";
			// PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터 바인딩
			pstmt.setInt(++cnt, status);
