...
public class BoardDAO {
	...
	public int getBoardCount(String keyfield,String keyword) throws Exception{
		...
		try {
			...
			if(rs.next()) { // 집합함수를 사용하면 값이 무조건 1개 생성되기 때문에 if문 사용
				count = rs.getInt(1);
			}
		}catch(Exception e) {
			throw new Exception(e);
	...
	public void deleteBoard(long board_num)throws Exception{
		...
		try {
			...
			// 좋아요 삭제               // foreign key 제약 조건에 걸려있는 경우 자식글부터 제거 후 부모글 삭제
			...
			// 채팅 메시지 삭제
			sql = "DELETE FROM zboard_chat WHERE chatroom_num IN (SELECT chatroom_num FROM zboard_chatroom WHERE board_num=?)";
	...
	public List<BoardVO> getListBoardFav(int start,int end,long mem_num)throws Exception{
		...
		try {
			// 커넥션풀로부터 커넥션 할당
			conn = DBUtil.getConnection();
			// SQL문 작성 (주의)zboard_fav의 회원번호(좋아요 클릭한 회원번호)로 검색되어야 하기 때문에 (알리아스를 사용해) f.mem_num으로 표기해야 함
			sql = "SELECT * FROM (SELECT a.*,rownum rnum FROM "
				+ "(SELECT * FROM zboard b JOIN zmember m "
	...
	public List<BoardReplyVO> getListReplyBoard(int start,int end,long board_num)throws Exception{
		...
		try {
			...
			while(rs.next()) {
				...
				reply.setRe_num(rs.getLong("re_num")); // 변수 board_num을 넣으면 오류가 날수도 안날 수도 있기 때문에 주의
	...
	public BoardReplyVO getReplyBoard(long re_num) throws Exception{
		...
		try {
			...
			if(rs.next()) {
				...
				reply.setRe_num(rs.getLong("re_num")); // 조건체크하기 위해 2개의 컬럼을 가져옴.
				reply.setMem_num(rs.getLong("mem_num"));
			}			
		} catch (Exception e) {
			throw new Exception(e);
	...
	public int getBoardCountByUser(String keyfield, String keyword, long mem_num) throws Exception {
		...
		try {
			...
			// SQL문 작성
			sql = "SELECT COUNT(*) FROM zboard JOIN (SELECT board_num, MAX(chat_num) chat_num FROM zboard_chatroom "
	...
	public List<BoardVO> getListBoardByUser(int start, int end, String keyfield, String keyword, long mem_num)throws Exception{
		...
		try {
			...
			sql = "SELECT * FROM (SELECT a.*, rownum rnum FROM (SELECT * FROM zboard "
				+ "JOIN (SELECT board_num, MAX(chat_num) chat_num FROM zboard_chatroom JOIN zboard_chat "
				+ "USING(chatroom_num) GROUP BY board_num) USING(board_num) "
				+ "LEFT OUTER JOIN (SELECT board_num, SUM(read_check) unread_cnt FROM zboard_chatroom "
				+ "JOIN zboard_chat USING(chatroom_num) "
	...
	public int getChatCount(String keyfield,String keyword, long board_num) throws Exception{
		...
		try {
			...
			if(keyword != null && !keyword.equals("")) {
				if(keyfield.equals("1")) sub_sql += "AND id LIKE '%' || ? || '%'";
				else if(keyfield.equals("2")) sub_sql += "AND message LIKE '%' || ? || '%'";
			}
			// SQL문 작성
			sql = "SELECT COUNT(*) FROM zboard_chat JOIN zboard_chatroom USING(chatroom_num) "
				+ "JOIN (SELECT SUM(read_check) cnt, MAX(chat_num) chat_num FROM zboard_chat "
				+ "GROUP BY chatroom_num) USING(chat_num) JOIN zmember USING(mem_num) "
	...
	// 채팅 - 채팅방 목록(검색 목록)
	public List<BoardChatVO> getChatList(int start, int end, String keyfield, String keyword, long board_num, long mem_num) throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			// SQL문 작성
			sql = "SELECT * FROM (SELECT a.*,rownum rnum FROM (SELECT * FROM zboard_chat JOIN "
				+ "zboard_chatroom c USING(chatroom_num) JOIN (SELECT MAX(chat_num) chat_num FROM "
				+ "zboard_chat GROUP BY chatroom_num) USING(chat_num) LEFT OUTER JOIN "
				+ "(SELECT SUM(read_check) unread_cnt, MAX(chat_num) chat_num FROM zboard_chat "
				+ "WHERE mem_num!=? GROUP BY chatroom_num) USING(chat_num) JOIN "
				+ "zmember z ON c.reader_num=z.mem_num "
				+ "WHERE board_num=? " + sub_sql + " ORDER BY chat_num DESC)a) "
				+ "WHERE rnum >= ? AND rnum <= ?";
			// PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터 바인딩
	...
	public int getBoardCountForMyChat(String keyfield,String keyword,long mem_num)throws Exception{
		...
		try {
			...
			// SQL문 작성
			sql = "SELECT COUNT(*) FROM zboard JOIN zmember USING(mem_num) JOIN "
				+ "zboard_chatroom USING(board_num) JOIN "
				+ "(SELECT MAX(chat_num) chat_num, chatroom_num FROM zboard_chat "
				+ "GROUP BY chatroom_num) USING(chatroom_num) WHERE reader_num=? " + sub_sql;			
			// PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터 바인딩
			pstmt.setLong(1, mem_num);
	...
	public List<BoardVO> getListBoardForMyChat(int start,int end,String keyfield,String keyword,long mem_num) throws Exception{
		...
		try {
			...
			sql = "SELECT * FROM (SELECT a.*, rownum rnum FROM (SELECT * FROM zboard JOIN "
				+ "zmember USING(mem_num) JOIN zboard_chatroom USING(board_num) JOIN "
				+ "(SELECT MAX(chat_num) chat_num, chatroom_num FROM zboard_chat "
				+ "GROUP BY chatroom_num) USING(chatroom_num) LEFT OUTER JOIN "
				+ "(SELECT SUM(read_check) unread_cnt, chatroom_num FROM zboard_chat "
				+ "WHERE mem_num!=? GROUP BY chatroom_num) USING(chatroom_num) "
				+ "WHERE reader_num=? " + sub_sql + " ORDER BY chat_num DESC)a) "
				+ "WHERE rnum >= ? AND rnum <= ?";
			// PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터 바인딩
	...
	public BoardChatRoomVO checkChatRoom(long board_num,long reader_num)throws Exception{
		...
		try {
			...
			// SQL문 작성
			sql = "SELECT c.*, "
				+ "(SELECT id FROM zmember WHERE mem_num=writer_num) writer_id, "
				+ "(SELECT id FROM zmember WHERE mem_num=reader_num) reader_id, "
				+ "z.title FROM zboard_chatroom c JOIN "
				+ "zboard z ON c.board_num=z.board_num "
	...
	// 채팅 - 채팅방 정보 반환 //(위와 받는 반환같은 같지만,매개변수가 다름)
	...
	// 채팅 - 채팅방 생성 // 위2개는 채팅방이 있을 경우, 이건 없을 경우 생성
	public void insertChatRoom(BoardChatRoomVO vo)throws Exception {
		...
		try {
			...
			conn = DBUtil.getConnection();
			// SQL문 작성
			sql = "INSERT INTO zboard_chatroom (chatroom_num,board_num,writer_num,reader_num) VALUES (zchatroom_seq.nextval,?,?,?)";
			// PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터 바인딩
	...
	public void insertChat(BoardChatVO chat) throws Exception{
		...
		try {
			...
			conn = DBUtil.getConnection();
			// SQL문 작성
			sql = "INSERT INTO zboard_chat (chat_num,chatroom_num,mem_num,message) VALUES (zchat_seq.nextval,?,?,?)";
			// PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터 바인딩
	...
	// 채팅 - 채팅메시지 읽기(목록) - 내가 쓴건 업데이트하지 않도록
	public List<BoardChatVO> getChatDetail(long user_num,long chatroom_num)throws Exception{
		...
		try {
			...
			// 내가 보낸 메시지가 아닌 상대방이 보낸 메시지를 읽었을 때 read_check를 1에서 0으로 변경
			sql = "UPDATE zboard_chat SET read_check=0 WHERE mem_num!=? AND chatroom_num=?";
			pstmt = conn.prepareStatement(sql);
			...
			DBUtil.executeClose(null, pstmt, null); // pstmt, pstmt2는 순서가 없지만 마지막에만 conn 정리
			DBUtil.executeClose(rs, pstmt2, conn); // pstmt2에 의해 rs가 만들어지므로 pstmt2를 정리할 때 rs도 같이 정리
		}
		return list;
	}
