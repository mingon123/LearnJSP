...
public class MemberDAO {
	...
	// ID,Email 중복 체크 및 로그인 처리
	public boolean checkUniqueInfo(String id, String email) throws Exception{
		// Inner Join하면 회원탈퇴 시 누락된 데이터가 발생할 수 있기 때문에 Outer Join을 해야 함
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		...
		// 둘 다 null일 경우 에러나게 하기위해 else문 작성X
		
		try {
			// 커넥션풀로부터 커넥션 할당
			conn = DBUtil.getConnection();
			sql = "SELECT * FROM zmember LEFT OUTER JOIN zmember_detail USING(mem_num) WHERE " + sub_sql;
	...
	public MemberVO checkMember(String id)throws Exception{
		...
		try {
			...
			conn = DBUtil.getConnection();
			// SQL문 작성
			sql = "SELECT * FROM zmember JOIN zmember_detail USING(mem_num) WHERE id=?";
			// PreparedStatement 객ㅊ ㅔ생성
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터 바인딩
			pstmt.setString(1, id);
			// SQL문 실행
	...
	public void updateMember(MemberVO member) throws Exception{
		...
		try {
			...
			conn = DBUtil.getConnection();
			// SQL문 작성
			sql = "UPDATE zmember_detail SET name=?,phone=?,zipcode=?,address1=?,address2=?,modify_date=SYSDATE WHERE mem_num=?";
			// PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터 바인딩
	...
	// 관리자
	// 전체 내용 개수,검색 내용 개수			keyfield:컬럼명,keyword:값
	public int getMemberCountByAdmin(String keyfield,String keyword) throws Exception{
		...
		try {
			...
			// SQL문 작성(LEFT OUTER JOIN : 탈퇴한 아이디까지 표시하기 위해 사용)
			...
			rs = pstmt.executeQuery();
			if(rs.next()) {
				count = rs.getInt(1); // 컬럼 인덱스(컬럼이 COUNT(*) 1개이기 때문에 사용, alias 안주고 대신 사용)
			}
		}catch(Exception e) {
			throw new Exception(e);
		}finally {
	...
	public List<MemberVO> getListMemberByAdmin(int start,int end,String keyfield,String keyword) throws Exception{
		...
		try {
			...
			sql = "SELECT * FROM (SELECT a.*, rownum rnum FROM "
				...
				+ "USING(mem_num) " + sub_sql + " ORDER BY reg_date DESC NULLS LAST)a) " // null값은 마지막에 표시
				+ "WHERE rnum >= ? AND rnum <= ?";
			// PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			if(keyword != null && !keyword.equals("")) {
